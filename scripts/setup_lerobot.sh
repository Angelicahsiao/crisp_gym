...
if [ -d "$LEROBOT_DIR" ]; then
    echo "lerobot already exists at $LEROBOT_DIR"
    echo "Pulling latest for rev $LEROBOT_REV..."
    cd "$LEROBOT_DIR"
    git fetch --tags origin
    git checkout "$LEROBOT_REV"
else
...
if grep -q 'requires-python = ">=3.12"' "$PYPROJECT"; then
    ...
    echo "$PYPROJECT already patched or has unexpected requires-python value"
fi
